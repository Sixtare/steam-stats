...
export function BottomNav({ activeView, setActiveView, onHome }: BottomNavProps) {
  const getTabClass = (tabName: string) => {
    const base = "w-9 h-9 flex items-center justify-center rounded-lg transition-all duration-300 ease-out cursor-pointer";
  ...
  return (
    <nav className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 bg-surface-container-low/95 backdrop-blur-2xl border border-outline-variant/30 rounded-xl p-1.5 flex items-center gap-1 shadow-2xl shadow-black/80 z-50 max-w-[90vw] shrink-0">
      {/* Home / New Analysis button */}
      <button 
        className="w-9 h-9 flex items-center justify-center rounded-lg text-on-surface-variant hover:text-white hover:bg-white/5 transition-all duration-300 cursor-pointer"
        onClick={onHome}
        title="New Analysis"
      >
      ...
      {/* Divider */}
      <div className="w-[1px] h-5 bg-outline-variant/30 mx-0.5 shrink-0" />
